...
| Cell area         | 0.202 mm2 (201453 um2) | measured |
| Sequential area   | 36.6 %                 | measured |
| Clock target      | 20.0 ns (50.0 MHz)     | config   |
| WNS / TNS         | 0.000 / 0.000 ns       | measured |
| Worst setup/hold  | +6.520 / +0.060 ns     | measured |
| Timing            | MET                    | measured |
| Achieved fmax     | 74.2 MHz               | derived: 1/(T-WNS) |
```

> Closed at 50 MHz with +6.52 ns setup slack ⇒ **74.2 MHz achievable**
> (+24.2 MHz headroom).
...
with self-consistent path arithmetic. The *measured* numbers (area, slack) are
the real ones in `gds_flow/`; `samples/clean_signoff.rpt` mirrors the real
+6.52 ns signoff so the "TIMING MET / implied fmax" path is demonstrated on the
true result.
