...
module soc_top #(
    ...
    logic imem_cen, dmem_rdy;
    riscv_pipeline #(.XLEN(32), .RESET_PC(RESET_PC), .SYNC_FETCH(SYNC_MEM != 0)) u_cpu (
        .clk, .rst_n,
        .imem_addr, .imem_cen, .imem_rdata, .imem_ready(1'b1),  // async or pipelined fetch
    ...
    // load memory-wait readiness to the CPU: peripherals answer combinationally
    // (ready=1); only RAM loads carry the SRAM read latency. Fetch uses imem_cen.
