...
// Synchronous write, combinational read with write-first bypass.
module regfile #(
    ...
    // x0 is always zero; write-first bypass so a read in the same cycle as a
    // write returns the new value (matches single-cycle WB semantics).
    always_comb begin
